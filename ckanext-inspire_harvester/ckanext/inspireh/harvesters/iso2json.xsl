...
rec["licence"] = """[<xsl:for-each select="gmd:identificationInfo/*/gmd:resourceConstraints/gmd:MD_LegalConstraints">"<xsl:value-of select="normalize-space(gmd:useLimitation/*)"/>"<xsl:if test="position() != last()">,</xsl:if></xsl:for-each>]""";
