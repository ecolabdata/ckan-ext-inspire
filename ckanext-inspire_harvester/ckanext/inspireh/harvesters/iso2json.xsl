...
<xsl:template match="gmd:MD_Metadata">
    <xsl:variable name="apos">'</xsl:variable>
  	<xsl:variable name="mdlang" select="normalize-space(gmd:language/gco:CharacterString)"/>
    ...
    rec["dataset-language"] = """[<xsl:for-each select="gmd:identificationInfo/*/gmd:language"> 
      "<xsl:value-of select="normalize-space(*/@codeListValue)"/>"<xsl:if test="position() != last()">,</xsl:if>
      </xsl:for-each>]""";
